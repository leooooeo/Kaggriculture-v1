...
def agent(obs):
    try:
        ...
        action = _deep_preempt(obs, action, step)
...
# --- Deep preempt: lead a depth-1 preempt opponent by selling premium goods
# --- even earlier. Pulls premium SELLs scheduled at step+1..step+DEPTH forward
# --- to the current step, repaying (removing) the duplicate order when we reach
# --- that future step. Total volume is unchanged; only timing moves earlier.
import os as _dp_os
_DEEP_ENABLED = True
_DEEP_DEPTH = 24
_DEEP_START = int(_dp_os.environ.get("DEEP_START", "24"))
_DEEP_STOP = int(_dp_os.environ.get("DEEP_STOP", "680"))
_DEEP_MAX_BATCH = int(_dp_os.environ.get("DEEP_MAXB", "30"))
_DEEP_MAX_CLONE_DISTANCE = 6
_DEEP_STATE = {
    0: {"last_step": -1, "sold_ahead": {}},
    1: {"last_step": -1, "sold_ahead": {}},
}


def _deep_state(obs, step):
    seat = _seat(obs)
    st = _DEEP_STATE[seat]
    if step == 0 or step < int(st.get("last_step", -1)):
        st = {"last_step": step, "sold_ahead": {}}
        _DEEP_STATE[seat] = st
    st["last_step"] = step
    return st


def _deep_future_sells(step, depth):
    out = {}
    for d in range(1, depth + 1):
        fs = step + d
        if fs >= len(_ACTIONS):
            break
        for raw in (_ACTIONS[fs].get("market") or []):
            if len(raw) >= 3 and raw[0] == "SELL" and raw[1] in _PREMIUM:
                out[(fs, raw[1])] = out.get((fs, raw[1]), 0) + max(0, int(raw[2]))
    return out


def _deep_preempt(obs, action, step):
    if not _DEEP_ENABLED:
        return action
    st = _deep_state(obs, step)
    action = _copy_action(action)
    market = list(action.get("market") or [])
    # 1) Repay: reduce this step's scheduled premium SELLs by what we already sold early for this step.
    due = {k: int(v) for k, v in dict(st["sold_ahead"].pop(step, {})).items() if int(v) > 0}
    if due:
        new_market = []
        for raw in market:
            order = list(raw)
            if len(order) >= 3 and order[0] == "SELL" and due.get(order[1], 0) > 0:
                item = order[1]; req = max(0, int(order[2]))
                cut = min(req, due[item]); req -= cut; due[item] -= cut
                if req <= 0:
                    continue
                order[2] = req
            new_market.append(order)
        market = new_market
    # 2) Pull forward future premium sells (depth>1 leads a depth-1 opponent).
    if (_DEEP_START <= step < _DEEP_STOP) and _clone_distance(obs) <= _DEEP_MAX_CLONE_DISTANCE and len(market) < 10:
        remaining = _projected_shed(obs, {"market": market})
        for raw in market:
            if len(raw) >= 3 and raw[0] == "SELL":
                remaining[raw[1]] = max(0, int(remaining.get(raw[1], 0) or 0) - max(0, int(raw[2])))
        future = _deep_future_sells(step, _DEEP_DEPTH)
        for (fs, item) in sorted(future.keys()):
            if len(market) >= 10:
                break
            want = future[(fs, item)]
            avail = max(0, int(remaining.get(item, 0) or 0))
            qty = min(want, avail, _DEEP_MAX_BATCH)
            if qty <= 0:
                continue
            market.append(["SELL", item, qty])
            remaining[item] = avail - qty
            st["sold_ahead"].setdefault(fs, {})
            st["sold_ahead"][fs][item] = st["sold_ahead"][fs].get(item, 0) + qty
    action["market"] = market[:10]
    return action
